...
$stmt = mysqli_prepare($conn, "SELECT * FROM accommodation WHERE AccommodationID = ?");
mysqli_stmt_bind_param($stmt, "i", $AccommodationID);
if (mysqli_stmt_execute($stmt)) {
    $accommodationResult = mysqli_stmt_get_result($stmt);
    $accommodationRow = mysqli_fetch_assoc($accommodationResult);
}
$pageTitle = $accommodationRow["Name"];
include __DIR__ . '/../includes/header.php';
// 1. CACHE SETUP ============================================
$cacheFile = __DIR__ . "/../cache/weather_{$AccommodationID}.json";
...
// Fetch media for THIS accommodation
$mediaStmt = mysqli_prepare($conn, "SELECT * FROM media WHERE EntityType = 'accommodation' AND EntityID = ?");
...
<section class="flex flex-col items-center lg:flex-row lg:items-start lg:justify-center lg:gap-10  px-5 my-5 opacity-0 translate-y-20 transition-all duration-1000 ease-out">
    ...
    $mediaPrimaryStmt = mysqli_prepare($conn, "SELECT * FROM media WHERE EntityType = 'accommodation' AND EntityID = ? AND IsPrimary = 1");
    mysqli_stmt_bind_param($mediaPrimaryStmt, "i", $AccommodationID);
    ...
    ?>
    <div><img src="../media/accommodations/<?= $mediaPrimaryRow["URL"] ?? 'default.jpg'; ?>" alt="<?= $mediaPrimaryRow['Caption']; ?>" class="object-cover rounded-lg w-[850px]"></div>
    <div class=" text-xl gap-5">
        <h1 class="text-5xl font-bold mb-2"><?= $accommodationRow["Name"] ?></h1>
        <p class="flex flex-col mb-2"><span class="text-primary font-semibold underline">Description: </span><?= $accommodationRow["Description"] ?></p>
        <p class="flex flex-col mb-2"><span class="text-primary font-semibold underline">City: </span><?= $accommodationRow["City"] ?></p>
        <p class="flex flex-col mb-2"><span class="text-primary font-semibold underline">Address: </span><?= $accommodationRow["Address"] ?></p>
        <?php if ($weatherData && isset($weatherData['main']['temp'])): ?>
            <p class="flex flex-col mb-2"><span class="text-primary font-semibold underline">Weather: </span>The temperature in <?= $accommodationRow["City"] ?> is:
